...
def verify(num):
    predList = []
    pred = plantFilter[plantFilter['cntntsNo'].isin([num])]

    # 햇빛
    if pred['lighttdemanddoCodeNm'].isin(['높은 광도(1,500~10,000 Lux)']).any():
        predList.append(1)
    elif pred['lighttdemanddoCodeNm'].isin(['중간 광도(800~1,500 Lux)']).any():
        predList.append(2)
    else:
        predList.append(3)

    # 물의 양
    if pred['watercycleAutumnCodeNm'].isin(['053001', '053002']).any():
        predList.append(1)
    elif pred['watercycleAutumnCodeNm'].isin(['053003']).any():
        predList.append(2)
    else:
        predList.append(3)

    # 겨울철 집 온도
    if pred['watercycleAutumnCodeNm'].isin(['057001']).any():
        predList.append(1)
    elif pred['watercycleAutumnCodeNm'].isin(['057002']).any():
        predList.append(2)
    elif pred['watercycleAutumnCodeNm'].isin(['057003', '057004']).any():
        predList.append(3)
    else:
        predList.append(4)

    # 사랑 줄 수 있는지
    if pred['managelevelCode'].isin(['089003']).any():
        predList.append(1)
    else:
        predList.append(2)
    ...
    #게으름
    if pred['managedemanddoCode'].isin(['058003', '058004']).any():
        predList.append(1)
    else:
        predList.append(2)

    #성격 급함
    if pred['grwtveCode'].isin(['090001', '090002']).any():
        predList.append(1)
    else:
        predList.append(2)

    # 향
    if pred['smellCode'].isin(['079004', '079003']).any():
        predList.append(1)
    else:
        predList.append(2)

    # 추위
    if pred['grwhTpCode'].isin(['082001', '082002']).any():
        predList.append(1)
    else:
        predList.append(2)
...
print("유사도 :" + str(cosine_similarity(pred, decodeData)))
